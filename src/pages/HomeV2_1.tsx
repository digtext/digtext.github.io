...
const BULLET_DEMO_CONTENT = `- Dig text enables readers to expand the parts of text they find valuable
  - Nesting has no limit — every layer is a choice the reader makes
    - They don't have to read everything to understand the context of what comes after
    - When you're done, just collapse it back
- Try dig text yourself — write with bullet indentation
  - Switch to the Raw text view above to play with it and see how it's written
    - This bullet format is a natural way to outline ideas at different depths
  - Use our LLM prompt below to automatically transform any text into dig text`;
...
Now convert the original text into "digText" format — a progressive, collapsed-by-default reading layout. The digText syntax uses bullet-list indentation:

- Top-level bullets are always visible
  - Indented sub-bullets are hidden by default — the reader clicks [+] to reveal them
    - Deeper indentation = deeper levels of detail

Each level of nesting adds more detail about its parent. If a reader skips a collapsed block, the surrounding bullets still make complete sense on their own.

1. Deconstruct the summaries you prepared in the 1st step into indented bullets. The shortest summary stays at the top level (visible by default); all other summaries are nested as indented sub-bullets.
2. Roughly: the top level of dig-text should be the shortest summary you already prepared in the 1st step. 2nd level the 2nd longest summary. 3rd level the 3rd longest summary. And so on. Nest all of the original information (not only summaries) in indented bullets. Nothing is cut — everything is preserved, just collapsed.
3. Use the progressive expansion principle. Spread sub-bullets evenly throughout the text.
4. After you finish, re-read only the top-level bullets. They must read as a coherent, complete summary of the original on their own. If they don't, rework it until they do.

Output only the converted text in bullet-list format. The output should be ready to paste directly into dig text.
    ...
    <div className="min-h-screen bg-white text-neutral-900 dark:bg-neutral-950 dark:text-neutral-50">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto px-6 pt-20 pb-28">
          ...
          <BulletDigTextReader content={BULLET_DEMO_CONTENT} />
